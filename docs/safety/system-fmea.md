...
| **`platform/pdf-viewer`** | **DECOMMISSIONED** Stream-rendering failure / empty canvas rendering | Visual component shows blank artifact, breaking the What-You-See-Is-What-You-Sign constraint. | ~~9~~ | ~~4~~ | ~~3~~ | **~~108~~** | **DECOMMISSIONED (2026-08-22):** Component officially removed from cluster architecture. Risk completely mitigated by transitioning to native browser HTML `<iframe>` object viewports, offloading binary stream parsing directly to the host OS thread to eliminate dependency-level canvas rendering bugs [fmea]. | — | — | — | **0** |
