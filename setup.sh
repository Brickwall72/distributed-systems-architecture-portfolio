...
if command -v docker &> /dev/null; then
    ...
    if [ "$OS_TYPE" == "Linux" ]; then
        if command -v apt-get &> /dev/null; then
            ...
            curl -fsSL https://docker.com | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg || true
            sudo apt-get install -y docker-ce docker-ce-cli containerd.io
            sudo systemctl start docker
            sudo systemctl enable docker
            sudo usermod -aG docker "$USER"
        else
            echo "ERROR: Unsupported Linux distribution. Please install Docker manually." >&2
            exit 1
        fi
        ...
        echo "ERROR: Automated Docker setup for Windows requires manual execution." >&2
        exit 1
    fi
fi
...
if command -v node &> /dev/null; then
    if CURRENT_NODE_VER=$(node -v 2>/dev/null | sed 's/v//'); then
...
# We run pnpm setup natively to anchor binary environment pathways on the host shell
pnpm setup

# --- STEP 5: WORKSPACE DEPENDENCY LINKING EXECUTION ---
echo "Executing pnpm workspace package compilation loops..."
# If this is your first local build, remove --frozen-lockfile for this run only
# to allow pnpm to physically generate your brand new pnpm-lock.yaml baseline file!
pnpm install
